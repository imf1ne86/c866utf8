...
Declare Function SayInUtf8(t As String) As WString Ptr
...
End Type
Dim tblUtf8 As Utf8Table, idx As Ulong, oneByteString As String * 1, twoByteString As String * 2
...
If Not ((inputFile = "") Or (outputFile = "") Or (inputFile = outputFile)) Then
  ...
  Dim isFileExist As Long
  isFileExist = FileExists(inputFile): ' Returns non-zero (-1) if the file exists, otherwise returns zero (0).
  If isFileExist <> 0 Then
    ' процесс конвертации
    SayIt("Начался процесс конвертации...")
    Dim fileSize As LongInt, filePos As LongInt, pieceOfFile As String * 1, killResult As Long
    killResult = Kill(outputFile): ' Returns zero (0) on success, or non-zero on error.
    Open inputFile For Binary As #1 Len = 1
    Open outputFile For Binary As #2 Len = 2
      fileSize = Lof(1): ' размер входящего файла (в байтах)
      filePos = Seek(1)
      While ((fileSize > 0) And (filePos <= fileSize))
        ...
        If tblUtf8.utf8Size(idx) = 1 Then
          oneByteString = Chr(tblUtf8.utf8Lower(idx))
          Put #2, Seek(2), oneByteString
        Else
          twoByteString = Chr(tblUtf8.utf8Lower(idx)) & Chr(tblUtf8.utf8Upper(idx))
...
Function SayInUtf8(txt As String) As WString Ptr
  Dim As WString Ptr result
  result = Allocate(256 * Len(WString))
  *result = txt
  Return result
End Function

Sub SayIt(txt As String, newline As Boolean = True)
  Dim pSay As WString Ptr = SayInUtf8(txt)
  If newline Then
    Print *pSay
  Else
    Print *pSay;
